from tkinter import *
import BackBookApp # Importing backend functions from another file which is querying SQLLite DB
...
def view_command():
    list1.delete(0,END)
    for row in BackBookApp.view():
        list1.insert(END,row)

def search_command():
    list1.delete(0,END)
    for row in BackBookApp.search(e1_title.get(),e2_author.get(),e3_year.get(),e4_isbn.get()):
        list1.insert(END,row)

def add_command():
    BackBookApp.insert(e1_title.get(),e2_author.get(),e3_year.get(),e4_isbn.get())
...
def delete_command():
    BackBookApp.delete(selected_row[0]) #pull first value from list that is id field

def update_command():
    BackBookApp.update(selected_row[0],e1_title.get(),e2_author.get(),e3_year.get(),e4_isbn.get())
...
#listbox
list1 = Listbox(win,height=7, width =35)
